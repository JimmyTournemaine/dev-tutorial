...
describe('REST API Tests', () => {
  ...
  it('should get a tutorial slides content', async () => {
  ...
  it('should start the given tutorial docker container', async function () {
    ...
    const start = await request(app).post('/api/tuto/dev/start') as StartResponse;
    ...
    await new Promise<void>((resolve, reject) => {
      ...
      const interval = setInterval(() => {
        void request(app).get(start.headers.location)
          .then((statusLocation) => {
  ...
  xit('should got an error trying to write a file in a docker container that do not exist', async () => {
    await request(app)
